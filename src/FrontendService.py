...
class FrontendService:
    ...
    def get_data_json(self, response_text, gpt_input_text_df):
        ...
        def get_explainability_json(response_text, source_text):
            def get_colors():
                return ['#df7f9f', '#bf7fff', '#7f9fff', '#7fdfff', '#9fffd7', '#bfffbf', '#dfff7f', '#ffdf7f', '#ffcf9f', '#ffbfbf']
            ...
            def get_explain_json(text, word_color_dict):
                ...
                for word in word_list:
                    if word.lower() in common_word_sequences:
        ...
        return response_text_with_footnote, source_text, {'response_json': response_json,
...
if __name__ == '__main__':
    ...
    paragraph2 = """
Source (1)
ChatGPT is a sibling model toÂ InstructGPT, which is trained to follow an instruction in a prompt and provide a detailed response.
- ChatGPT is sensitive to tweaks to the input phrasing or attempting the same prompt multiple times. For example, given one phrasing of a question, the model can claim to not know the answer, but given a slight rephrase, can answerÂ correctly.
ChatGPT is fine-tuned from a model in the GPT-3.5 series, which finished training in early 2022. You can learn more about the 3.5 seriesÂ here. ChatGPT and GPT-3.5 were trained on an Azure AI supercomputingÂ infrastructure.
Todayâs research release of ChatGPT is the latest step in OpenAIâsÂ iterative deploymentÂ of increasingly safe and useful AI systems. Many lessons from deployment of earlier models like GPT-3 and Codex have informed the safety mitigations in place for this release, including substantial reductions in harmful and untruthful outputs achieved by the use of reinforcement learning from human feedbackÂ (RLHF).
